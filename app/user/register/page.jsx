...
import { Sparkles, Lock, User, AlertTriangle, Wand } from 'lucide-react'
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      setError('Your incantations do not match. Please try again.')
      return
    }
    
    // Validate password strength
    if (password.length < 6) {
      setError('Your incantation is too weak. It must be at least 6 characters long.')
      return
    }
      ...
      <div className="relative overflow-hidden p-6 rounded-lg bg-gradient-to-br from-gray-900 to-gray-800 shadow-xl">
        ...
        <div className="relative z-10 max-w-md mx-auto">
          <div className="flex items-center justify-center mb-6">
            <Wand className="text-purple-400 mr-3" size={24} />
            <h2 className="text-3xl font-bold text-white">Join the Magical Order</h2>
            <Sparkles className="text-purple-400 ml-3" size={24} />
          </div>
          
          <p className="text-center text-gray-300 mb-8">
            Create your magical identity to begin your journey
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-gray-300 mb-2 text-sm font-medium" htmlFor="password">
                Create Your Secret Incantation
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <input
                  ...
                  className="w-full pl-10 p-3 bg-gray-800 border border-gray-700 text-white rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300"
                  type="password"
                  placeholder="Create a powerful incantation"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                />
              </div>
              <p className="mt-1 text-xs text-gray-400">Your incantation must be at least 6 characters long</p>
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-400">
              Already a wizard?{' '}
              <Link href="/round2/login" className="text-purple-400 hover:text-purple-300 transition-colors duration-300">
                Enter the Realm
              </Link>
            </p>
          </div>
